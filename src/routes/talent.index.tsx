...
import { createTalentDirectoryLoader, TALENT_PAGE_SIZE } from "./-talent-loaders";
...
import { parseTalentSearch, talentListingLoaderDeps } from "@/lib/talent-search";
          ...
          meta: [
            {
              ...
              title: match.status === "notFound" ? m.notFound_heading() : m.talentDirectory_title(),
            },
          ],
        },
...
function TalentDirectoryPage() {
  ...
  const selectedOnPage = page?.data.some(
    (candidate) =>
      candidate.handle === search.selectedTalent || candidate.id === search.selectedTalent,
  );
  ...
  if (restricted) {
    return (
      <>
        <RestrictedTalentDirectory boardName={seo.boardName} signedIn={user !== null} />
      </>
    );
  }
