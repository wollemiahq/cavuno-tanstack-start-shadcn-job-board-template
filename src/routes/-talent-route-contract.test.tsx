...
const getTalentIndexPage = vi.fn<TalentDirectoryRouteDependencies["getTalentIndexPage"]>();
const getTalentProfilePage = vi.fn<TalentProfileRouteDependencies["getTalentProfilePage"]>();
...
function talentLoaderContext(deps: { q?: string; skill?: string; page?: number }) {
  return {
...
describe("talent directory route — query and capability contracts", () => {
  ...
  it("offers a signed-in candidate the add-company path instead of another sign-in", async () => {
    await renderRouted(<RestrictedTalentDirectory boardName="Acme Careers" signedIn />);
  ...
  it("turns a disabled or missing directory into the route not-found outcome", async () => {
    getTalentIndexPage.mockRejectedValue(apiError(404, "talent_directory_not_found"));
...
describe("canonical talent profile route", () => {
  it("retrieves the requested public handle and retains canonical metadata", async () => {
    const loaderData = await profileLoader(profileLoaderContext("ada-lovelace"));
  ...
  it("routes an employer without talent access to pricing", async () => {
    ...
    expect(screen.getByRole("link", { name: "Message" })).toHaveAttribute("href", "/employers");
  });
  ...
  it("hides the Message action from a candidate viewer (no cold-messaging) ", async () => {
    ...
    expect(container.querySelector("[data-slot='talent-profile-actions']")).toBeNull();
  ...
  it("offers an upgrade link when an employer has access but no message credits", async () => {
    ...
    expect(screen.getByRole("link", { name: "Upgrade to message" })).toHaveAttribute(
      "href",
      "/employers",
    );
  ...
  it("renders the named profile without an unlock gate", async () => {
    ...
    expect(screen.getByRole("heading", { level: 1, name: "Ada Lovelace" })).toBeVisible();
    expect(screen.queryByRole("heading", { name: "Unlock this profile" })).toBeNull();
    expect(screen.queryByRole("heading", { name: "Talent access required" })).toBeNull();
  });
});
...
describe("opaque talent profile unlock gate", () => {
  it("shows how full the profile is behind every gate state", async () => {
    ...
    for (const surface of ["unlock_needed", "out_of_unlocks", "no_plan"] as const) {
  ...
  it("confirms an unlock when the employer still has credits", async () => {
    ...
    fireEvent.click(within(dialog).getByRole("button", { name: "Unlock profile" }));
  ...
  it("sends viewers without a talent plan to /employers", async () => {
    ...
    expect(screen.getByRole("link", { name: "View talent plans" })).toHaveAttribute(
      "href",
      "/employers",
    );
  });
