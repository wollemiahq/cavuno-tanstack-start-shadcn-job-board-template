...
const mocks = vi.hoisted(() => ({ invalidate: vi.fn(), resetPassword: vi.fn() }));
...
function validateSearch(search: Record<string, unknown>) {
  ...
  if (typeof validate !== "function") {
    throw new Error("The reset-password route must validate its search parameters");
  }
...
describe("/auth/reset-password continuation", () => {
  ...
  it("keeps the destination when requesting a replacement reset link", () => {
    ...
    vi.spyOn(Route, "useSearch").mockReturnValue({ token: undefined, returnTo });
    const ResetPasswordPage = Route.options.component;
    if (!ResetPasswordPage) throw new Error("The reset-password route needs a component");
  ...
  it("keeps the destination on sign-in after a successful reset", async () => {
    const returnTo = "/jobs?q=design&selectedJob=product-designer";
    vi.spyOn(Route, "useSearch").mockReturnValue({ token: "reset-token", returnTo });
  ...
  it("recovers when the password update rejects unexpectedly", async () => {
    vi.spyOn(Route, "useSearch").mockReturnValue({ token: "reset-token", returnTo: "/account" });
