...
function validateSearch(
  ...
  if (typeof validate !== "function") {
    throw new Error("The auth callback route must validate its search parameters");
  }
...
describe("auth callback continuation", () => {
  ...
  ] as const)("validates %s before its loader consumes returnTo", (_name, route) => {
    expect(
  ...
  it.each([
    ...
    vi.spyOn(route, "useLoaderData").mockReturnValue({ status: "invalid" } as never);
    vi.spyOn(route, "useSearch").mockReturnValue({
      token: "one-time-token",
      returnTo,
    });
    const Component = route.options.component;
    if (!Component) throw new Error("The auth callback route needs a component");
