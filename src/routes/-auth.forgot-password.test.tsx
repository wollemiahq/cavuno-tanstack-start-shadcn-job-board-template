...
function validateSearch(search: Record<string, unknown>) {
  ...
  if (typeof validate !== "function") {
    throw new Error("The forgot-password route must validate its search parameters");
  }
...
describe("/auth/forgot-password continuation", () => {
  it("sanitizes and retains an internal candidate destination", () => {
    expect(validateSearch({ returnTo: "/jobs?q=design&selectedJob=product-designer" })).toEqual({
  ...
  it("keeps the destination on the confirmation link back to sign in", async () => {
    ...
    if (!ForgotPasswordPage) throw new Error("The forgot-password route needs a component");
