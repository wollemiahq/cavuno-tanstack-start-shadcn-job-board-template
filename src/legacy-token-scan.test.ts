...
const RHEA_PILOT = new Set([
  ...
  "src/components/candidate-route-state.tsx",
  ...
  "src/components/board/candidate-account-shell.tsx",
  ...
  return importSpecifiers(source).filter((specifier) => {
    const target = /^[@#]\//.test(specifier)
      ...
      : specifier.startsWith(".")
        ...
        : null;
    return /^src\/components\/(?:base|application|untitled-ui)(?:\/|$)/.test(target ?? "");
  }).length;
}
...
function directBaseUiImportCount(file: string, source: string): number {
  ...
  return importSpecifiers(source).filter((specifier) => {
    if (specifier !== BASE_UI_PACKAGE && !specifier.startsWith(`${BASE_UI_PACKAGE}/`)) {
      return false;
    }
    ...
    if (specifier === "@base-ui/react/select" && !source.includes("SelectPrimitive.Portal")) {
      return false;
    }
    return specifier === BASE_UI_PACKAGE || PORTALED_BASE_UI_SUBPATH_RE.test(specifier);
  }).length;
}
    ...
    for (const [file, current] of Object.entries(counts)) {
      ...
      for (const key of ["imports", "icons", "tokens"] as const) {
        if (current[key] > allowed[key]) {
          regressions.push(`${file} — ${key}: ${current[key]} > baseline ${allowed[key]}`);
        }
      }
    }
    expect(
...
describe("pure legacy scanner fixtures", () => {
  ...
  it("rejects Base UI root-barrel imports outside owned UI components", () => {
    expect(
      directBaseUiImportCount("src/routes/example.tsx", `import { Button } from '@base-ui/react'`),
    ).toBe(1);
  });
  ...
  it("rejects side-effect Base UI imports outside owned UI components", () => {
    expect(
      directBaseUiImportCount("src/components/example.tsx", `import '@base-ui/react/button'`),
    ).toBe(1);
  });
  ...
  it("permits the owned Sheet only when its portal root carries the Rhea scope", () => {
    expect(
      directBaseUiImportCount(
...
describe("foundation dependency boundary", () => {
  it("introduces neither a direct Radix dependency nor a Radix source import", () => {
    ...
    expect(Object.keys(directDependencies).filter((name) => name.startsWith("@radix-ui/"))).toEqual(
      [],
    );
