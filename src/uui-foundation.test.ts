...
describe("Rhea foundation and inherited Untitled UI compatibility CSS", () => {
  ...
  it("keeps pre-Rhea names out of inherited application source", () => {
    ...
    const SKIP =
      /\.test\.tsx?$|routeTree\.gen\.ts$|resolved\.ts$|components\/(?:ui|layout|search-results|messages)\/|components\/(?:alert-manager|auth-form|avatar-upload|candidate-route-state|candidate-shell|danger-zone|education-section|experience-section|job-alert-floating-prompt|languages-section|messages-nav-link|notification-settings|profile-form|resume-upload|rhea-auth-pilot|skills-section)\.tsx$|components\/board\/(?:alert-signup-form|alerts-band|candidate-account-shell|company-search-controls|company-search-detail-state|company-search-page|company-search-result|company-search-result-detail|home-landing|job-card|job-search-page|job-search-result|job-search-result-detail|job-search-detail-state|jobs-filter-toolbar|jobs-results-bar|jobs-search-controls|save-job-button|talent-profile-content|talent-search-controls|talent-search-detail-state|talent-search-page|talent-search-result|talent-search-result-detail)\.tsx$|routes\/(?:account(?:\.|\.tsx)|alerts\.|auth\.|me\.|settings\.tsx)/;
