...
describe('shadcn Rhea foundation', () => {
  ...
  it('scopes colliding Rhea color utilities away from legacy surfaces', () => {
    ...
    expect(untitledTheme).toContain(
      '--background-color-primary: var(--color-bg-primary)',
    )
    expect(styles).not.toMatch(
      /@theme[^}]*--(?:color|background-color)-primary:/s,
    )
    expect(styles).toMatch(
      /\.rhea-theme\s*{[^}]*--background-color-primary:\s*var\(--primary\)/s,
    )
  })
  ...
  it('tells future agents to expand Rhea while legacy only shrinks', () => {
    ...
    expect(agents).toMatch(/portaled Rhea primitives[\s\S]*prohibited/i)
