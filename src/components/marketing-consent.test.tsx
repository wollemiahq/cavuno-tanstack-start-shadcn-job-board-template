...
const mocks = {
  invalidate: vi.fn(),
  setMarketingConsent: vi.fn<UpdateMarketingConsent>().mockResolvedValue(updatedConsent),
};
...
describe("sign-up marketing checkbox", () => {
  it("renders nothing and sends no flag when the surface is off", async () => {
    ...
    expect(document.querySelector('[data-test="sign-up-marketing-consent"]')).toBeNull();
  ...
  it("defaults unticked and submits marketingConsent: false untouched", async () => {
    ...
    const checkbox = document.querySelector('[data-test="sign-up-marketing-consent"]');
    ...
    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ marketingConsent: false }));
  });
  ...
  it("submits marketingConsent: true after an affirmative tick", async () => {
    ...
    fireEvent.click(document.querySelector('[data-test="sign-up-marketing-consent"]')!);
    fillRequiredFields();
    fireEvent.click(screen.getByRole("button", { name: "Create account" }));
    await screen.findByText("Check your email");

    expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ marketingConsent: true }));
  });
});

describe("settings marketing consent row", () => {
  const settings = (consent: Parameters<typeof MarketingConsentSettingsView>[0]["consent"]) => (
  ...
  it("renders unticked for null (never decided) and withdrawn alike", async () => {
    ...
    expect(document.querySelector('[data-test="marketing-consent-toggle"]')).toHaveAttribute(
      "aria-checked",
      "false",
    );
    unmount();
  ...
  it("grants on tick and withdraws on untick", async () => {
    const { unmount } = await renderRouted(settings(null));
    fireEvent.click(document.querySelector('[data-test="marketing-consent-toggle"]')!);
    await vi.waitFor(() =>
    ...
    const toggle = document.querySelector('[data-test="marketing-consent-toggle"]')!;
