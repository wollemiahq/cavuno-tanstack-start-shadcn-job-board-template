 ...
 *
 *  - The role-named variants map to EXACTLY the Untitled UI token the design
 *    system assigns them (display → display-md, heading1 → display-sm, … ,
 *    body → text-md). heading1-4 are byte-aligned with the vendored `.prose`
 ...
 *    hatch merged through `cx` (so an override wins).
 */
...
describe('Text — role-named variant → UUI token mapping', () => {
  it('maps each heading variant to its locked UUI size + semibold + primary color', () => {
    const cases = [
      { variant: 'display', as: 'h1', size: 'text-display-md' },
      { variant: 'heading1', as: 'h1', size: 'text-display-sm' },
      { variant: 'heading2', as: 'h2', size: 'text-display-xs' },
    ...
    for (const { variant, as, size } of cases) {
      ...
      // The hero `display` role steps up to display-lg on desktop so a
      // marketing hero keeps its prominence; every other role is a single
      // locked step.
      if (variant === 'display') expect(el.className).toContain('md:text-display-lg')
      cleanup()
      void container
    }
  })

  it('heading1-4 match the vendored prose h1-h4 tokens byte-for-byte', () => {
    // The prose stylesheet renders h1=display-sm, h2=display-xs, h3=xl,
    // h4=lg (all font 600, color text-primary). <Text> heading variants must
    ...
    const proseTokens = {
      heading1: 'text-display-sm',
      heading2: 'text-display-xs',
      heading3: 'text-xl',
      heading4: 'text-lg',
    } as const
  ...
  it('maps body-family variants to their default token + color', () => {
    ...
    expect(secondary.className).toContain('text-md')
    expect(secondary.className).toContain('text-tertiary')
    cleanup()

    render(<Text variant="error">boom</Text>)
    const error = screen.getByText('boom')
    expect(error.className).toContain('text-sm')
    expect(error.className).toContain('text-error-primary')
  })
})
...
describe('Text — `as` decouples the element from the variant', () => {
  it('renders the requested element regardless of variant', () => {
    ...
    // An h2 can carry the heading1 (display-sm) visual role.
    render(
    ...
    expect(el.className).toContain('text-display-sm')
    cleanup()
...
describe('Text — size / bold tune body-family only', () => {
  it('applies the size token, overriding the variant default', () => {
    ...
    for (const { size, token } of sizes) {
      ...
      // The size wins: the default text-md is not left dangling alongside a
      // different size (cx/tailwind-merge dedupes the font-size group).
      if (token !== 'text-md') expect(el.className).not.toContain('text-md')
      cleanup()
    }
  })
...
describe('Text — truncate + className escape hatch', () => {
  ...
  it('className merges and a color override wins over the variant default', () => {
    render(
      <Text variant="heading2" as="h2" className="mb-6 text-error-primary">
        override
      </Text>,
    )
    const el = screen.getByText('override')
    expect(el.className).toContain('mb-6')
    expect(el.className).toContain('text-error-primary')
    // cx (tailwind-merge) drops the losing default text color.
    expect(el.className).not.toContain('text-primary')
    // The locked size is untouched.
    expect(el.className).toContain('text-display-xs')
  })
  ...
  it('a responsive size added via className survives (base variant token kept)', () => {
    ...
    render(
      <Text variant="heading2" as="h1" className="md:text-display-sm">
        responsive
      </Text>,
    )
    const el = screen.getByText('responsive')
    expect(el.className).toContain('text-display-xs')
    expect(el.className).toContain('md:text-display-sm')
  })
})
