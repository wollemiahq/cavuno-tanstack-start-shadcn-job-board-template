...
describe("CompanySearchResult", () => {
  it("uses one canonical anchor with visible selected state and real company facts", () => {
    ...
    const { container } = render(<CompanySearchResult vm={vm} selected onActivate={onActivate} />);
    ...
    expect(screen.getByText("Research tools for ambitious engineering teams.")).toBeVisible();
  ...
  it("falls back to initials and omits missing description and zero-job noise", () => {
    ...
    expect(screen.queryByText("Research tools for ambitious engineering teams.")).toBeNull();
