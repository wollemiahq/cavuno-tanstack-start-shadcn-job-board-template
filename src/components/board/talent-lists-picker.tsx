...
import { createTalentList, type ActionResult, type TalentListRecord } from "@/server/employers";
        ...
        <DropdownMenuContent align="start" className="w-72">
          {lists.map((list) => (
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={() => selectList(list.id === selectedListId ? null : list)}
            >
              <span className="min-w-0 truncate">{talentListDisplayName(list.name)}</span>
            </DropdownMenuCheckboxItem>
          ))}
