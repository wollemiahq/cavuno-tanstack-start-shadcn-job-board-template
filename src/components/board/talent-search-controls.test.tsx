...
describe("TalentSearchControls", () => {
  ...
  it("restores both URL-backed fields when browser history changes", () => {
    const { rerender } = render(
      <TalentSearchControls q="designer" skill="Figma" labels={labels} onSubmit={vi.fn()} />,
    );
    ...
    expect(screen.getByRole("searchbox", { name: "Candidate name or headline" })).toHaveValue(
      "researcher",
    );
    expect(screen.getByRole("textbox", { name: "Skill" })).toHaveValue("Accessibility");
  });
});
