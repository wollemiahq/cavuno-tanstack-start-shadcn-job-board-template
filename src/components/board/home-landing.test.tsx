...
describe("HomeLanding — latest jobs carry detail links (read.jobs doctor)", () => {
  it("links each job to its typed detail route on the home page", async () => {
    ...
    expect(link.getAttribute("href")).toBe("/companies/technova-labs/jobs/senior-backend-engineer");
  });

  it("previews the latest real fixture roles rather than invented marketing content", async () => {
    renderLanding(baseProps);

    expect(await screen.findByRole("link", { name: "Senior Backend Engineer" })).toBeTruthy();
    expect(screen.getByRole("link", { name: "Product Designer" })).toBeTruthy();
    expect(screen.getByRole("link", { name: "Machine Learning Engineer" })).toBeTruthy();
  });
...
describe("HomeLanding — hiring index", () => {
  it("presents the real hiring companies and counts as one named navigation region", async () => {
    ...
    expect(within(index).getByRole("link", { name: "Fieldstone Robotics" })).toHaveAttribute(
      "href",
      "/companies/fieldstone-robotics",
    );
    expect(within(index).getByRole("link", { name: "Brightpath Health" })).toHaveAttribute(
      "href",
      "/companies/brightpath-health",
    );
    expect(within(index).getByRole("link", { name: "TechNova Labs" })).toHaveAttribute(
      "href",
      "/companies/technova-labs",
    );
    expect(within(index).getByRole("link", { name: "Harborline Analytics" })).toHaveAttribute(
      "href",
      "/companies/harborline-analytics",
    );
...
describe("HomeLanding — empty board", () => {
  it("stays a useful landing page without presenting zero as a stat or a failed-search message", async () => {
    ...
    expect(screen.getByText("Check back soon for new opportunities.")).toBeTruthy();
    expect(screen.getByRole("link", { name: "Create an employer profile" })).toHaveAttribute(
      "href",
...
describe("HomeLanding — dual-path sign-up band", () => {
  ...
  it("shows ONLY the candidate card when employers are disabled", async () => {
    ...
    expect(screen.queryByRole("link", { name: "Create an employer profile" })).toBeNull();
  });
  ...
  it("shows ONLY the employer card when candidates are disabled", async () => {
    ...
    expect(screen.queryByRole("link", { name: "Create a candidate profile" })).toBeNull();
  });
