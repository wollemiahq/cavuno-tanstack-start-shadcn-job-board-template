...
const authorName = "Avery Montgomery-Smythe, Principal Editorial Research Fellow";
...
const post = {
  ...
  featureImageCaption: "The maintainers trace one component from shared token to product pattern.",
  customExcerpt: "How a mature team keeps decisions legible without slowing delivery.",
  readingTimeMin: 14,
...
describe("BlogArticleContent — complete article presentation", () => {
  it("uses the Page family, Typeset body, captioned image, and complete author links", async () => {
    ...
    expect(main).toContainElement(screen.getByRole("heading", { level: 1, name: post.title }));
    ...
    expect(screen.getByRole("img", { name: post.featureImageAlt! })).toBeVisible();
    expect(
      screen.getByText("The maintainers trace one component from shared token to product pattern.")
        .tagName,
    ).toBe("FIGCAPTION");
    ...
    expect(container.querySelector('a[href="https://avery.example"]')).toBeVisible();
    expect(container.querySelector('a[href="https://x.com/avery"]')).toBeVisible();
    expect(container.querySelector('a[href="https://www.linkedin.com/in/avery"]')).toBeVisible();
    expect(container.querySelector('a[href="https://github.com/avery"]')).toBeVisible();
    expect(
  ...
  it("does not turn a null API body into a blank article", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "This article has no published body" }),
    ).toBeVisible();
    expect(
      screen.getByText("The article record exists, but its content is not available."),
    ).toBeVisible();
