...
describe("JobSearchResult", () => {
  it("uses one canonical anchor with selected, focusable card semantics", () => {
    const onActivate = vi.fn((event: React.MouseEvent<HTMLAnchorElement>) =>
      event.preventDefault(),
    );
    const { container } = render(<JobSearchResult vm={vm} selected onActivate={onActivate} />);

    const link = screen.getByRole("link", { name: /Product designer/i });
    expect(link).toHaveAttribute("href", "/companies/acme/jobs/product-designer");
  ...
  it("renders honest compact metadata and a transparent paid-placement label", () => {
    ...
    expect(screen.getByText("Own discovery and the product design system.")).toBeVisible();
