...
describe("CandidateAccountShell", () => {
  ...
  it("owns the account page landmark while leaving headings to route content", async () => {
    ...
    const sidebar = container.querySelector('[data-slot="candidate-account-sidebar"]');
    const content = container.querySelector('[data-slot="candidate-account-content"]');
  ...
  it("navigates between account sections through TanStack Router without a document load", async () => {
    ...
    await waitFor(() => expect(router.state.location.pathname).toBe("/account/saved"));
    expect(screen.getByRole("heading", { name: "Saved jobs destination" })).toBeVisible();
  });
});
