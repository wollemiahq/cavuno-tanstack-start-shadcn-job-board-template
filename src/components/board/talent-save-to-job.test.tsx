...
describe('TalentSaveToJob', () => {
  it('asks which job when the directory is unbound', () => {
    render(
      <TalentSaveToJob slug="acme" jobs={jobs} candidateBoardUserId="bu_ada" />,
    );

    expect(screen.getByRole('combobox', { name: 'Save to job' })).toBeVisible();
    expect(screen.getByRole('button', { name: 'Save to job' })).toBeEnabled();
  });
  ...
  it('saves in one click on a bound list and can start already saved', () => {
    ...
    expect(screen.queryByRole('combobox', { name: 'Save to job' })).toBeNull();
