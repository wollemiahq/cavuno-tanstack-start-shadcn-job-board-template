...
describe('tinybirdProxyTarget', () => {
  it('maps /t/* onto the Tinybird API host, preserving path and query', () => {
    expect(
      tinybirdProxyTarget(
        new URL('https://demo.cavuno.app/t/v0/events?name=analytics_events&token=p.x'),
      ),
    ).toBe('https://api.us-east.aws.tinybird.co/v0/events?name=analytics_events&token=p.x')
  })

  it('proxies only below /t — the path prefix is stripped exactly once', () => {
    expect(tinybirdProxyTarget(new URL('http://localhost:4199/t/v0/events'))).toBe(
