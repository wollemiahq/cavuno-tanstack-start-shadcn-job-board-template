...
import { seniorityFromSelection, selectionFromSeniority } from "./seniority-filter";
...
describe("seniorityFromSelection", () => {
    ...
    it("returns the selected levels for a concrete selection", () => {
        expect(seniorityFromSelection(new Set(["senior", "lead"]))).toEqual(["senior", "lead"]);
    });
...
describe("selectionFromSeniority", () => {
    ...
    it("maps an absent filter to an empty (cleared) selection", () => {
        expect(seniorityFromSelection(selectionFromSeniority(undefined))).toBeUndefined();
    });
});
